...
void set_sizescombobox(int font_idx)
{
	...
	if(first_time||font0.sizes.size()||font.sizes.size())
	{
		...
		if(font.sizes.size())//bitmap font
			sizes=&font.sizes[0], sizes_size=(int)font.sizes.size();
		else//true type font
			sizes=truetypesizes, sizes_size=truetypesizes_size;

		int nsizes=(int)SendMessageW(hFontsizecb, CB_GETCOUNT, 0, 0);//clear sizes combobox
		...
		size_t result=std::find(sizes, sizes+sizes_size, font_size)-sizes;//find current fontsize
		if(result<(size_t)sizes_size)//found
...
{
	size_t idx2=SendMessageW(hFontcombobox, CB_GETCURSEL, 0, 0);
	if(idx2!=(size_t)font_idx)
	{
		set_sizescombobox((int)idx2);
		font_idx=(int)idx2;
	}
...
void print_text()
{
	ptrdiff_t size=SendMessageW(hTextbox, WM_GETTEXTLENGTH, 0, 0);
	if(size>0)
	{
		...
		DrawTextW(ghMemDC, text.c_str(), (int)size, &r, DT_NOCLIP);
		...
		for(int k=0;k<image_size;++k)//reverse alpha algorithm
		{
			...
			{
				if((uint32_t)color==(primarycolor&colormask))
					image[k]=primarycolor;
				else if((uint32_t)color==(secondarycolor&colormask))
